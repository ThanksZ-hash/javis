...
import { extractHwpText, isBinaryHwp } from "@/lib/hwp";
...
export async function POST(req: NextRequest) {
  ...
  if (!mimeType && ext !== HWP_EXTENSION) {
    return NextResponse.json({
      supported: false,
      brief: "AI 브리핑은 현재 PDF, 이미지, HWP 파일만 지원합니다.",
    });
  }

  if (doc.file_size && doc.file_size > MAX_BRIEF_FILE_SIZE) {
    return NextResponse.json({
      supported: false,
